...
static int SyncLorenzMaster(Master *self)
{
    ...
    self->um = ((self->alpha - 0.01) * self->c + 0.028) * self->x1m +
               (0.01 * self->c + self->alpha - 0.001) * self->x2m -
               0.001 * self->x1m * self->x3m;
...
static int SyncLorenzSlave(Slave *self, void *m)
{
    ...
    self->us = ((0.01 - self->alpha) * self->c - 0.028) * self->x1s -
               (0.01 * self->c + self->alpha - 0.001) * self->x2s;
    self->u = self->us + um;
...
{
    if (NULL == (*self = malloc(sizeof(Master))))
        return -1;
...
{
    if (NULL == (*self = malloc(sizeof(Slave))))
        return -1;
    (*self)->x1s = slave_x1s;
    (*self)->x2s = slave_x2s;
    (*self)->x3s = slave_x3s;
